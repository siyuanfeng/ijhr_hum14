...
\markboth{Siyuan Feng, Eric Whitman, X Xinjilefu, Chris Atkeson} {Optimization Based Full Body Control for the Atlas Robot}
...
The proposed full body controller was tested on Boston Dynamics's Atlas robot,
...
Desired CoM motioni was represented with a quintic spline. 
...
On the downside, our approach required more (and more difficult) input from 
...
frame. 
Only the first and third rows in \eref{eq:cart_rot}, that corresponded to 
...
$R$ was constructed from a basis of three orthogonal unit vectors including 
...
To avoid chase conditions, this modification was disabled if there was not 
...
\label{sec:dis}
During early robot development before the Trials, we found using ID alone is 
...
static tasks such as manipulation and ladder climbing in the Trials. 
...
Unmeasured friction and stiction forces degrade performance of torque control. 
